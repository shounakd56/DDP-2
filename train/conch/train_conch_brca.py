...
With Semantic Wasserstein Routing (SWR), CONCH encoder, BRCA-specific
...
** Key differences vs the RCC version: **
- 2 classes (IDC, ILC) instead of 3 (KICH, KIRP, KIRC).
- Slide labels come from a CSV (cols: submitter_id, label) instead of
  per-class folder names.
- BRCA-specific routing prompts (MORPHOLOGICAL_PATTERNS,
  MICROENVIRONMENT_COMPONENTS, DIAGNOSTIC_DISCRIMINATORS) and
  class-level descriptions (IDC, ILC).
- Indexing walks a flat slide directory (no per-class subdirs).
- Memory / barycenter / SWR / UOT / aggregation code unchanged.

** v3.1 BarycentreMemory RESUME FIX is preserved verbatim. **

HOW TO RUN:
-----------
Basic (fresh run):
    python train_conch_brca.py

Override key paths at command line:
    python train_conch_brca.py \\
        --brca_root      /data/tcga_brca/WSI \\
        --conch_feats_root /data/tcga_brca/tcga_feats \\
        --label_csv      /data/tcga_brca/brca_labels.tsv \\
        --k 16 \\
        --epochs 50

Resume from last checkpoint:
    python train_conch_brca.py --resume

Resume from a specific checkpoint:
    python train_conch_brca.py --resume_from /path/to/checkpoint.pth

Rebuild train/val/test splits (re-index + re-split):
    python train_conch_brca.py --rebuild_splits

REQUIRED FILE STRUCTURE:
------------------------
5x patches  : <brca_root>/<slide_id>/5x/*.{jpg,png}
           OR <brca_root>/pyramid/<slide_id>/5x/*.{jpg,png}

20x features: <conch_feats_root>/<slide_id>/20x/*.pt   (per-patch .pt files)
           OR <conch_feats_root>/pyramid/<slide_id>/20x/*.pt
           OR <conch_feats_root>/<slide_id>.pt           (single (N,D) tensor)
...
ENV VARIABLES:
--------------
    HF_TOKEN   — Hugging Face token for MahmoodLab/conch download
...
os.environ["CUDA_VISIBLE_DEVICES"] = "1"   # or remove entirely
...
class Config:
    ...
    # ====== BRCA PATHS  — replace with real paths ======
    brca_root        = "/home/datasets/tcga_brca"          # DUMMY PATH
    conch_feats_root = "/home/datasets/tcga_brca_conch_feats"   # DUMMY PATH
    label_csv        = "/data/ankit/BRCA_MODELS/16_shots/tcga_brca_labels.csv"  # DUMMY PATH
    ...
    ablation_name         = "brca_2class_swr_CONCH"
    disable_uot           = False
...
def apply_shot_adaptive_config(cfg):
    ...
    if cfg.save_dir                     is None: cfg.save_dir                     = f"./fewshot_ckpt_conch_brca_k{k}_20x_full_seed_114"
...
# ===========================================================================
# BRCA-specific Prompt Definitions  (IDC vs ILC)
# ===========================================================================

MORPHOLOGICAL_PATTERNS = {
    "Infiltrative tumor regions": {
        "description": (
            "At low power, irregularly shaped, ill-defined tumor regions "
            "invade surrounding breast tissue, displacing or distorting "
            "adjacent ducts and lobules."),
        "magnification": "5x", "class_association": "IDC", "diagnostic_weight": 0.95},
    "Dense tumor cellularity": {
        "description": (
            "Solid nests or trabecular cords of tumor cells occupy large "
            "portions of the breast parenchyma, replacing normal "
            "lobular architecture."),
        "magnification": "5x", "class_association": "IDC", "diagnostic_weight": 0.9},
    "Glandular or trabecular formations": {
        "description": (
            "Tumor architecture features malformed gland-like ducts or "
            "linear trabeculae of malignant epithelial cells embedded "
            "within fibrotic stroma."),
        "magnification": "both", "class_association": "IDC", "diagnostic_weight": 0.95},
    "Necrotic zones with peripheral viable rim": {
        "description": (
            "Focal areas of necrosis, often centrally located within "
            "tumor nests, with a surrounding rim of viable hyperchromatic "
            "tumor cells showing mitotic activity."),
        "magnification": "both", "class_association": "IDC", "diagnostic_weight": 0.85},
    "Single-file growth pattern": {
        "description": (
            "Tumor cells infiltrate in linear, single-cell-wide rows "
            "between stromal collagen fibers, forming the classic "
            "Indian-file architecture characteristic of lobular carcinoma."),
        "magnification": "both", "class_association": "ILC", "diagnostic_weight": 1.0},
    "Diffuse infiltration without mass formation": {
        "description": (
            "Tumor cells are widely dispersed across the stroma in a "
            "discohesive pattern, lacking mass-forming architecture and "
            "minimally distorting pre-existing tissue landmarks."),
        "magnification": "5x", "class_association": "ILC", "diagnostic_weight": 0.95},
    "Scattered isolated tumor cells": {
        "description": (
            "Neoplastic cells appear loosely distributed as single "
            "entities or very small groups, without clustering into "
            "nests, sheets, or glandular structures."),
        "magnification": "both", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Loss of cohesive structures": {
        "description": (
            "Absence of glandular, trabecular, or solid nest formations, "
            "with minimal disruption of pre-existing breast lobular and "
            "ductal architecture."),
        "magnification": "5x", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Perivascular and periductal infiltration": {
        "description": (
            "Tumor cells wrap concentrically around existing ducts and "
            "vessels in a targetoid fashion, without destroying them and "
            "preserving normal tissue outlines."),
        "magnification": "both", "class_association": "ILC", "diagnostic_weight": 0.9},
}
...
MICROENVIRONMENT_COMPONENTS = {
    "Desmoplastic stromal response": {
        "description": (
            "Fibroblast-rich, densely fibrotic stroma surrounds tumor "
            "nests and glandular elements, producing the firm consistency "
            "characteristic of invasive ductal carcinoma."),
        "magnification": "both", "class_association": "IDC", "diagnostic_weight": 0.7},
    "Reactive fibrosis": {
        "description": (
            "Dense collagen bundles with stromal retraction and "
            "scattered lymphocytes surround invasive tumor regions."),
        "magnification": "both", "class_association": "IDC", "diagnostic_weight": 0.6},
    "Peritumoral lymphocytic aggregates": {
        "description": (
            "Clusters of immune cells at tumor-stroma borders suggesting "
            "host inflammatory response to the invasive tumor front."),
        "magnification": "both", "class_association": "shared", "diagnostic_weight": 0.5},
    "Distorted stromal architecture": {
        "description": (
            "Stroma shows reactive changes and desmoplasia due to "
            "infiltrative and space-occupying tumor growth, with loss of "
    ...
    "Mixed inflammatory infiltrate near necrosis": {
        "description": (
            "Neutrophils and macrophages infiltrate around necrotic "
            "zones, indicating active tumor-host interaction and tissue "
            "breakdown."),
        "magnification": "20x", "class_association": "shared", "diagnostic_weight": 0.45},
    "Sparse stromal reaction": {
        "description": (
            "Fibrous stroma is loose, delicate, and minimally reactive, "
            "contrasting sharply with the desmoplastic response typical "
            "of ductal carcinoma."),
        "magnification": "both", "class_association": "ILC", "diagnostic_weight": 0.7},
    "Preserved background architecture": {
        "description": (
            "Pre-existing breast tissue landmarks including normal ducts "
            "and lobules remain visible despite tumor infiltration, a "
            "hallmark of lobular growth."),
        "magnification": "5x", "class_association": "ILC", "diagnostic_weight": 0.6},
    "Normal breast lobules and ducts": {
        "description": (
            "Preserved terminal duct-lobular units with bilayered "
            "epithelium consisting of inner luminal and outer "
            "myoepithelial cells, surrounded by intralobular stroma."),
        "magnification": "both", "class_association": "shared", "diagnostic_weight": 0.3},
    "Adipose tissue": {
        "description": (
            "Mature adipocytes with peripherally displaced nuclei "
            "forming the fatty background of the breast parenchyma."),
...
DIAGNOSTIC_DISCRIMINATORS = {
    "Pleomorphic tumor cells": {
        "description": (
            "Tumor cells display marked variation in size and shape, "
            "with irregular contours and disorganized cytoarchitecture, "
            "characteristic of high-grade ductal carcinoma."),
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 1.0},
    "Irregular nuclear contours": {
        "description": (
            "Nuclei show jagged, wrinkled borders with coarse clumped "
            "chromatin and frequent atypical mitotic figures."),
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.95},
    "High nuclear-to-cytoplasmic ratio": {
        "description": (
            "Tumor cells have large hyperchromatic nuclei and scant "
            "cytoplasm, reflecting aggressive cellular proliferation "
            "in invasive ductal carcinoma."),
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.9},
    "Prominent eosinophilic nucleoli": {
        "description": (
            "Nuclei feature conspicuous central eosinophilic nucleoli, "
            "characteristic of high-grade invasive ductal carcinoma."),
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.9},
    "Increased mitotic activity": {
        "description": (
            "Numerous mitotic figures, including abnormal tripolar and "
            "ring forms, are visible particularly at the invasive front "
            "of the tumor."),
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.9},
    "Ghost cell debris in necrotic foci": {
        "description": (
            "Necrotic areas contain pyknotic nuclei, faded eosinophilic "
            "cytoplasm, and fragmented cell remnants forming amorphous "
            "debris."),
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.7},
    "Small uniform tumor cells": {
        "description": (
            "Tumor cells are monomorphic and small, with round-to-oval "
            "nuclei, inconspicuous nucleoli, and bland cytology typical "
            "of lobular carcinoma."),
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 1.0},
    "Round or oval bland nuclei": {
        "description": (
            "Nuclear morphology is bland and regular, with smooth "
            "contours, fine evenly dispersed chromatin, and minimal "
            "atypia."),
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.95},
    "Minimal nuclear pleomorphism": {
        "description": (
            "Cellular features are remarkably uniform with very limited "
            "variability in nuclear size, shape, or staining intensity."),
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Intracytoplasmic mucin vacuoles": {
        "description": (
            "Clear cytoplasmic vacuoles, sometimes containing a central "
            "eosinophilic dot, displace nuclei to create a targetoid or "
            "signet-ring-like appearance characteristic of lobular "
            "carcinoma."),
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 1.0},
    "Lack of glandular lumen formation": {
        "description": (
            "No glandular lumina or epithelial polarity are observed, a "
            "key distinguishing feature from invasive ductal carcinoma."),
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Nuclear monotony": {
        "description": (
            "Cells exhibit strikingly uniform nuclear size, shape, and "
            "chromatin pattern, reflecting low-grade lobular morphology."),
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Clear cytoplasm with discrete vacuoles": {
        "description": (
            "Cytoplasm appears pale to clear with discrete intracellular "
            "vacuoles, sometimes mistaken for benign histiocytic or "
            "stromal tissue at low magnification."),
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.85},
    "Discohesive single cells": {
        "description": (
            "Tumor cells lack intercellular adhesion and junctions, "
            "appearing as isolated entities — the morphological "
            "correlate of E-cadherin loss in lobular carcinoma."),
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.95},
}
...
CLASS_KNOWLEDGE_V2 = {
    "IDC": {"primary": (
        "Invasive ductal carcinoma of the breast characterized by "
        "infiltrative tumor regions, malformed glandular or trabecular "
        "architecture, pleomorphic tumor cells with high nuclear-to-"
        "cytoplasmic ratio, prominent nucleoli, frequent mitoses, and "
        "a marked desmoplastic stromal reaction.")},
    "ILC": {"primary": (
        "Invasive lobular carcinoma of the breast characterized by "
        "single-file linear cords of small uniform discohesive tumor "
        "cells infiltrating diffusely through stroma, with bland round "
        "nuclei, intracytoplasmic mucin vacuoles, lack of gland "
        "formation, and preserved background architecture.")},
}
...
# ===========================================================================
# BRCA SWR Prompt Learner
# ===========================================================================
class BRCASWRPromptLearner(nn.Module):
    def __init__(self, conch_model, class_names, num_soft_tokens=16, config=None):
        ...
        logger.info(
            f"BRCASWRPromptLearner: {self.num_routing_concepts} concepts "
            f"(IDC={n_idc}, ILC={n_ilc}, Shared={n_x})")
    ...
    def encode_text_with_soft_prompts(self, input_ids, soft_ctx):
        ...
        for i in range(soft_ctx.shape[0]):
            ...
            x       = (prompt + pos_emb.unsqueeze(0)).permute(1, 0, 2)
            ...
            x    = text_model.ln_final(x.permute(1, 0, 2))
            feat = x[:, new_eos, :]
            tp   = text_model.text_projection
            feat = feat @ tp if (isinstance(tp, nn.Parameter) or torch.is_tensor(tp)) else tp(feat)
...
class DeepPromptedCONCHVisionTransformer(nn.Module):
    ...
    def _forward_block(self, block, x):
        if self.use_gradient_checkpointing and self.training:
            return grad_checkpoint(lambda inp: block(inp), x, use_reentrant=False)
        return block(x)
    ...
    def _project_to_embed(self, tokens):
        ...
        try:
            ...
            if hasattr(vm, 'proj_contrast') and vm.proj_contrast is not None:
                ...
                pooled = pooled @ proj if (isinstance(proj, nn.Parameter) or torch.is_tensor(proj)) else proj(pooled)
            return pooled
        except Exception:
    ...
    def forward(self, pixel_values, scale_idx=0):
        ...
        if x.dim() == 4:
            x = x.flatten(2).transpose(1, 2).contiguous() if x.shape[1] == D else x.reshape(B, -1, D).contiguous()
        elif x.dim() == 3 and x.shape[-1] != D:
        ...
        if hasattr(self.trunk, 'pos_drop'):  x = self.trunk.pos_drop(x)
        if hasattr(self.trunk, 'patch_drop'): x = self.trunk.patch_drop(x)
        if hasattr(self.trunk, 'norm_pre'):   x = self.trunk.norm_pre(x)
...
# ===========================================================================
# Main Model — MultiScaleMUOT_CONCH (BRCA)
# ===========================================================================
    ...
    def __init__(self, config):
        ...
        try:
            conch_model, self.conch_preprocess = create_model_from_pretrained(
                model_cfg=config.conch_model_name, checkpoint_path=config.conch_checkpoint,
        ...
        if config.num_patch_tokens is None: config.num_patch_tokens = (img_sz // 16) ** 2
        if config.vit_hidden_dim   is None: config.vit_hidden_dim   = trunk.embed_dim
        ...
        self.nested_prompt_learner = BRCASWRPromptLearner(
...
def _read_label_csv(label_csv_path):
    """Read CSV/TSV with `submitter_id` and `label` columns.

    Auto-detects delimiter.  Returns dict patient_id -> label_string.
    """
...
def index_brca_dataset(brca_root, conch_feats_root, label_csv_path, class_to_idx,
                       scales=('5x', '20x'), min_patches_per_scale=None):
    """Walk BRCA slide directories, look up class label from CSV."""
    ...
    feats_root_p = Path(conch_feats_root)
    indexed      = []
        ...
        for s in scales:
            if s == '5x':
                ...
                if feat_dir is not None:
                    ...
                    single_pt = feats_root_p / f"{sid}.pt"
                    if single_pt.exists():
...
def make_patient_level_splits(indexed_data, train_ratio=0.7, val_ratio=0.15,
                              test_ratio=0.15, seed=114, num_classes=2):
    ...
    def _load(self, scale_item):
        if isinstance(scale_item, str) and scale_item.endswith('.pt'):
            ...
            for p in scale_item:
                ...
                if   feat.dim() == 1:                              features.append(feat)
                elif feat.dim() == 2 and feat.shape[0] == 1:      features.append(feat.squeeze(0))
                else: raise RuntimeError(f"Unexpected per-patch feature shape {feat.shape} from {p}")
            return torch.stack(features, dim=0) if features else torch.zeros(0, self.config.embedding_dim)
        else:
            patches = []
...
# ===========================================================================
def parse_args():
    p = argparse.ArgumentParser(description="MultiScale MUOT-CONCH BRCA 2-class (IDC/ILC)")
    ...
    p.add_argument('--conch_feats_root', type=str, default=None, help='CONCH 20x features root')
...
def main(cfg=None, args=None):
    ...
    if args.conch_feats_root is not None: cfg.conch_feats_root   = args.conch_feats_root
    ...
    print("\n" + "=" * 80)
    print("MULTI-SCALE MUOT-CONCH — TCGA-BRCA 2-class (IDC/ILC) v3.1")
    ...
    print(f"  Feats root        : {cfg.conch_feats_root}")
    ...
    all_indexed = index_brca_dataset(
        cfg.brca_root, cfg.conch_feats_root, cfg.label_csv, cfg.class_to_idx,
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        torch.cuda.manual_seed_all(114)
