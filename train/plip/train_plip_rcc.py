...
With Semantic Wasserstein Routing (SWR), PLIP encoder, RCC-specific
...
- 20× patches now loaded from pre-extracted .pt feature files
  (path: <plip_feats_root>/<class>/pyramid/<class>/<slide_id>/20x/*.pt).
- Bypasses PLIP encoder for 20×; raw images still used for 5×.
...
Backbone: PLIP (Hugging Face Transformers CLIPModel) – 5x images encoded
on-the-fly, 20x features loaded from disk.
"""
...
class Config:
    ...
    plip_feats_root = "/home/datasets/tcga_rcc/plip_feats"
    class_dir_names = {
        "KICH": "kich",
        "KIRP": "kirp",
        "KIRC": "kirc",
    ...
    ablation_name = "rcc_3class_swr_PLIP"
    disable_uot = False
...
def apply_shot_adaptive_config(cfg):
    ...
    if cfg.save_dir is None:
        cfg.save_dir = f"./fewshot_ckpt_plip_rcc_k{k}_20x_full_seed_109"
...
class RCCSWRPromptLearner(nn.Module):
    def __init__(self, plip_model, class_names, num_soft_tokens=16, config=None):
        ...
        logger.info(
            f"RCCSWRPromptLearner (PLIP): {self.num_routing_concepts} concepts "
    ...
    def encode_text_with_soft_prompts(self, input_ids, soft_ctx):
        ...
        for i in range(soft_ctx.shape[0]):
            ...
            # Preserve EOS after inserting soft prompts.  Long pathology prompts
            # are truncated before EOS instead of dropping EOS accidentally.
        ...
        for slide_dir in tqdm(slide_dirs, desc=f"  {cls_name}"):
            ...
            for s in scales:
                if s == '5x':
                    ...
                    feat_dir = (Path(plip_feats_root) / cls_dir / "pyramid"
                    ...
                    if feat_dir.exists():
                        ...
                        single_pt = Path(plip_feats_root) / f"{slide_dir.name}.pt"
                        if single_pt.exists():
...
class MultiScaleWSIDataset(Dataset):
    ...
    def _coerce_plip_feature(self, feat, src=""):
        ...
        if feat.dim() == 1 and feat.shape[0] == self.config.embedding_dim:
            return feat.contiguous()
        raise RuntimeError(f"Unexpected PLIP feature tensor shape {tuple(feat.shape)} from {src}; "
    ...
    def _load(self, scale_item):
        if isinstance(scale_item, str) and scale_item.endswith('.pt'):
            ...
            return (torch.stack(features, dim=0).float() if features
                    else torch.zeros(0, self.config.embedding_dim, dtype=torch.float32))
        else:
            patches = []
...
def main(cfg=None, args=None):
    ...
    print("\n" + "=" * 80)
    print("MULTI-SCALE MUOT-PLIP — TCGA-RCC 3-class (KICH/KIRP/KIRC) v3.1")
    ...
    all_indexed = index_rcc_dataset(
        ...
        plip_feats_root=cfg.plip_feats_root)
    if not all_indexed:
