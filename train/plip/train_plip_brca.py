"""
v3.1 — TCGA-BRCA 2-class (IDC / ILC)
With Semantic Wasserstein Routing (SWR), PLIP encoder, BRCA-specific
hierarchical prompts, CSV-driven labels, automatic patient-level
train/val/test splits, class-imbalance-aware sampling, and full
RESUME-FROM-CHECKPOINT support.

Backbone: PLIP (Hugging Face Transformers CLIPModel) – 5x images encoded on-the-fly,
20x features loaded from disk.

Key differences vs original CONCH version:
- PLIP instead of CONCH (visual & text)
- Feature path: /home/datasets/tcga_brca_plip_feats
- PLIP-specific Hugging Face tokenizer & text model
- Same routing, UOT, barycenter, aggregation, SWR, SWA logic.

HOW TO RUN:
-----------
Basic (fresh run):
    python train_plip_brca.py

Override key paths at command line:
    python train_plip_brca.py \\
        --brca_root      /data/tcga_brca/WSI \\
        --plip_feats_root /home/datasets/tcga_brca_plip_feats \\
        --label_csv      /data/tcga_brca/brca_labels.tsv \\
        --k 16 \\
        --epochs 50

Resume from last checkpoint:
    python train_plip_brca.py --resume

Resume from a specific checkpoint:
    python train_plip_brca.py --resume_from /path/to/checkpoint.pth

Rebuild train/val/test splits (re-index + re-split):
    python train_plip_brca.py --rebuild_splits

REQUIRED FILE STRUCTURE:
------------------------
5x patches  : <brca_root>/<slide_id>/5x/*.{jpg,png}
20x features: <plip_feats_root>/<slide_id>/20x/*.pt   (per-patch .pt files)
           OR <plip_feats_root>/<slide_id>.pt           (single (N,D) tensor)

Label CSV   : TSV/CSV with at least two columns:
                submitter_id  — TCGA patient barcode (e.g. TCGA-D8-A3Z6)
                label         — one of {"IDC", "ILC"}
...
# ============================================================================
# PLIP IMPORTS (Hugging Face Transformers, not OpenCLIP)
# ============================================================================
try:
    from transformers import CLIPModel, CLIPProcessor, CLIPTokenizerFast
except ImportError:
    raise ImportError(
        "transformers is not installed. Install with: pip install transformers huggingface_hub"
    )
...
def _get_attr(obj, name, default=None):
    """Return an attribute if it exists; otherwise return default."""
    return getattr(obj, name, default)


def _load_plip_hf(config):
    """Load PLIP directly from Hugging Face Transformers.

    The public PLIP checkpoint `vinid/plip` is a Transformers `CLIPModel`, not
    an OpenCLIP-pretrained tag.  Loading through `open_clip.create_model_*` can
    fail on many installed open_clip versions with: `Pretrained value ... is not
    a known tag`.  This loader avoids OpenCLIP completely.
    """
    ckpt = str(config.plip_checkpoint).replace("hf-hub:", "")
    model = CLIPModel.from_pretrained(ckpt)
    try:
        processor = CLIPProcessor.from_pretrained(ckpt)
    except Exception as e:
        logger.warning(f"CLIPProcessor.from_pretrained({ckpt}) failed: {e}. Continuing without processor.")
        processor = None
    return model, processor, processor


def _get_plip_tokenizer(config):
    """Return the Hugging Face tokenizer matching the PLIP checkpoint."""
    ckpt = str(config.plip_checkpoint).replace("hf-hub:", "")
    return CLIPTokenizerFast.from_pretrained(ckpt)


# ===========================================================================
# Tokenizer compatibility shim (Hugging Face tokenizer)
# ===========================================================================
def _extract_input_ids(tokenizer_output):
    """Return input_ids from HF BatchEncoding/dict/namespace/tensor outputs."""
    if torch.is_tensor(tokenizer_output):
        return tokenizer_output

    # transformers.BatchEncoding is dict-like, but not always an actual dict,
    # and accessing `.shape` on it raises AttributeError.  Always extract
    # input_ids explicitly before any tensor shape checks.
    try:
    ...
    ids = getattr(tokenizer_output, "input_ids", None)
    if ids is not None:
        return ids

    data = getattr(tokenizer_output, "data", None)
    if isinstance(data, dict) and "input_ids" in data:
        return data["input_ids"]

    raise TypeError(
        f"Tokenizer output has no input_ids. Got type={type(tokenizer_output)}"
    )


def _safe_tokenize(texts, tokenizer, context_length=77):
    """Tokenize strings into a padded/truncated LongTensor of input_ids.

    Works with Hugging Face CLIPTokenizer/CLIPTokenizerFast, transformers
    BatchEncoding, dict-like outputs, and old callable tokenizers that return
    tensors directly.
    """
    ...
    try:
        ...
        # Fallback for tokenizers that do not accept HF keyword arguments.
        enc = tokenizer(texts)

    tokens = _extract_input_ids(enc)
...
def grab_gpu_memory(device="cuda:0", fraction=0.05):
    ...
    n_elements = int(free_mem * fraction) // 4
    try:
...
# ===========================================================================
# Configuration  (BRCA 2-class — IDC vs ILC)
# ===========================================================================
class Config:
    gpu_device_id = 0

    # ====== BRCA PATHS (PLIP-specific) ======
    brca_root        = "/home/datasets/tcga_brca"                # WSI images
    plip_feats_root  = "/home/datasets/tcga_brca_plip_feats"     # PLIP 20x features
    label_csv        = "/data/ankit/BRCA_MODELS/16_shots/tcga_brca_labels.csv"
    class_to_idx     = {"IDC": 0, "ILC": 1}
    idx_to_class     = {v: k for k, v in class_to_idx.items()}
    class_names      = ["IDC", "ILC"]
    num_classes      = 2

    # Splits
    split_json   = "./brca_data_splits.json"
    train_ratio  = 0.70
    val_ratio    = 0.15
    test_ratio   = 0.15
    split_seed   = 82

    # ===== MODEL: PLIP (Hugging Face Transformers CLIPModel) =====
    plip_model_name = "vinid/plip"
    plip_checkpoint = "vinid/plip"              # Hugging Face Transformers checkpoint
    hf_auth_token   = None                     # not needed for PLIP
    plip_img_size   = 224
    embedding_dim   = 512                      # CLIP projection dimension
    context_length  = 77                       # PLIP/CLIP text context length
    ...
    ablation_name         = "brca_2class_swr_PLIP"
    disable_uot           = False
...
def apply_shot_adaptive_config(cfg):
    ...
    if cfg.memory_num_prototypes    is None: cfg.memory_num_prototypes    = min(max(int(math.sqrt(k)*2), 2), 12)
    if cfg.few_shot_k_memory        is None: cfg.few_shot_k_memory        = k
    if cfg.memory_ema_momentum      is None: cfg.memory_ema_momentum      = min(0.999, 0.97 + 0.001*k)
    if cfg.memory_ema_min_momentum  is None: cfg.memory_ema_min_momentum  = 0.5
    if cfg.memory_ema_warmup_epochs is None: cfg.memory_ema_warmup_epochs = max(10, min(25, k*2))
    if cfg.memory_ema_update_every  is None: cfg.memory_ema_update_every  = max(3, min(10, k//2))
    if cfg.memory_ema_noise_std     is None: cfg.memory_ema_noise_std     = max(0.002, 0.01/math.sqrt(k/8))
    if cfg.bary_outer_iters         is None:
        avg = max(1, k // cfg.memory_num_prototypes)
        cfg.bary_outer_iters = min(20, max(5, avg*3))
    if cfg.bary_euclidean_mix           is None: cfg.bary_euclidean_mix           = max(0.1, 0.4 - 0.015*k)
    if cfg.label_smoothing              is None: cfg.label_smoothing              = max(0.1, min(0.35, 0.35 - 0.005*k))
    if cfg.agg_dropout                  is None: cfg.agg_dropout                  = max(0.3, min(0.7, 0.7 - 0.01*k))
    if cfg.weight_decay                 is None: cfg.weight_decay                 = max(0.05, min(0.2, 0.2 - 0.003*k))
    if cfg.learning_rate                is None: cfg.learning_rate                = min(2e-3, 8e-4*math.sqrt(k/8))
    if cfg.grad_clip                    is None: cfg.grad_clip                    = max(0.5, 1.0 - 0.02*k)
    if cfg.gradient_accumulation_steps  is None: cfg.gradient_accumulation_steps  = max(4, min(12, total_train))
    if cfg.num_epochs                   is None: cfg.num_epochs                   = max(40, min(100, int(70+(8-k)*2)))
    if cfg.early_stopping_patience      is None: cfg.early_stopping_patience      = max(12, cfg.num_epochs//3)
    if cfg.mixup_alpha                  is None: cfg.mixup_alpha                  = min(0.5, 0.15+0.015*k)
    if cfg.prototype_diversity_weight   is None: cfg.prototype_diversity_weight   = min(0.1, 0.02*math.sqrt(k/8))
    if cfg.prototype_contrastive_weight is None: cfg.prototype_contrastive_weight = min(0.05, 0.01*math.sqrt(k/8))
    if cfg.memory_replay_ratio          is None: cfg.memory_replay_ratio          = max(0.0, 0.3 - 0.015*k)
    if cfg.rdrop_alpha                  is None: cfg.rdrop_alpha                  = max(0.5, min(3.0, 3.0 - 0.1*k))
    if cfg.patch_dropout_rate           is None: cfg.patch_dropout_rate           = max(0.1, min(0.4, 0.4 - 0.01*k))
    if cfg.feature_noise_std            is None: cfg.feature_noise_std            = max(0.01, min(0.05, 0.05 - 0.001*k))
    if cfg.min_train_epochs             is None: cfg.min_train_epochs             = max(15, cfg.num_epochs//3)
    if cfg.swa_start_epoch              is None: cfg.swa_start_epoch              = max(20, int(cfg.num_epochs*0.4))
    if cfg.train_passes_per_epoch       is None: cfg.train_passes_per_epoch       = max(1, min(4, 32//total_train))
    if cfg.save_dir                     is None: cfg.save_dir                     = f"./fewshot_ckpt_plip_brca_k{k}_20x_full_seed_82"
...
# ===========================================================================
# BRCA-specific Prompt Definitions
# ===========================================================================
MORPHOLOGICAL_PATTERNS = {
    "Infiltrative tumor regions": {
        "description": "At low power, irregularly shaped, ill-defined tumor regions invade surrounding breast tissue, displacing or distorting adjacent ducts and lobules.",
        "magnification": "5x", "class_association": "IDC", "diagnostic_weight": 0.95},
    "Dense tumor cellularity": {
        "description": "Solid nests or trabecular cords of tumor cells occupy large portions of the breast parenchyma, replacing normal lobular architecture.",
        "magnification": "5x", "class_association": "IDC", "diagnostic_weight": 0.9},
    "Glandular or trabecular formations": {
        "description": "Tumor architecture features malformed gland-like ducts or linear trabeculae of malignant epithelial cells embedded within fibrotic stroma.",
        "magnification": "both", "class_association": "IDC", "diagnostic_weight": 0.95},
    "Necrotic zones with peripheral viable rim": {
        "description": "Focal areas of necrosis, often centrally located within tumor nests, with a surrounding rim of viable hyperchromatic tumor cells showing mitotic activity.",
        "magnification": "both", "class_association": "IDC", "diagnostic_weight": 0.85},
    "Single-file growth pattern": {
        "description": "Tumor cells infiltrate in linear, single-cell-wide rows between stromal collagen fibers, forming the classic Indian-file architecture characteristic of lobular carcinoma.",
        "magnification": "both", "class_association": "ILC", "diagnostic_weight": 1.0},
    "Diffuse infiltration without mass formation": {
        "description": "Tumor cells are widely dispersed across the stroma in a discohesive pattern, lacking mass-forming architecture and minimally distorting pre-existing tissue landmarks.",
        "magnification": "5x", "class_association": "ILC", "diagnostic_weight": 0.95},
    "Scattered isolated tumor cells": {
        "description": "Neoplastic cells appear loosely distributed as single entities or very small groups, without clustering into nests, sheets, or glandular structures.",
        "magnification": "both", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Loss of cohesive structures": {
        "description": "Absence of glandular, trabecular, or solid nest formations, with minimal disruption of pre-existing breast lobular and ductal architecture.",
        "magnification": "5x", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Perivascular and periductal infiltration": {
        "description": "Tumor cells wrap concentrically around existing ducts and vessels in a targetoid fashion, without destroying them and preserving normal tissue outlines.",
        "magnification": "both", "class_association": "ILC", "diagnostic_weight": 0.9},
}

MICROENVIRONMENT_COMPONENTS = {
    "Desmoplastic stromal response": {
        "description": "Fibroblast-rich, densely fibrotic stroma surrounds tumor nests and glandular elements, producing the firm consistency characteristic of invasive ductal carcinoma.",
        "magnification": "both", "class_association": "IDC", "diagnostic_weight": 0.7},
    "Reactive fibrosis": {
        "description": "Dense collagen bundles with stromal retraction and scattered lymphocytes surround invasive tumor regions.",
        "magnification": "both", "class_association": "IDC", "diagnostic_weight": 0.6},
    "Peritumoral lymphocytic aggregates": {
        "description": "Clusters of immune cells at tumor-stroma borders suggesting host inflammatory response to the invasive tumor front.",
        "magnification": "both", "class_association": "shared", "diagnostic_weight": 0.5},
    "Distorted stromal architecture": {
        "description": "Stroma shows reactive changes and desmoplasia due to infiltrative and space-occupying tumor growth, with loss of normal lobular structure.",
        "magnification": "5x", "class_association": "IDC", "diagnostic_weight": 0.65},
    "Mixed inflammatory infiltrate near necrosis": {
        "description": "Neutrophils and macrophages infiltrate around necrotic zones, indicating active tumor-host interaction and tissue breakdown.",
        "magnification": "20x", "class_association": "shared", "diagnostic_weight": 0.45},
    "Sparse stromal reaction": {
        "description": "Fibrous stroma is loose, delicate, and minimally reactive, contrasting sharply with the desmoplastic response typical of ductal carcinoma.",
        "magnification": "both", "class_association": "ILC", "diagnostic_weight": 0.7},
    "Preserved background architecture": {
        "description": "Pre-existing breast tissue landmarks including normal ducts and lobules remain visible despite tumor infiltration, a hallmark of lobular growth.",
        "magnification": "5x", "class_association": "ILC", "diagnostic_weight": 0.6},
    "Normal breast lobules and ducts": {
        "description": "Preserved terminal duct-lobular units with bilayered epithelium consisting of inner luminal and outer myoepithelial cells, surrounded by intralobular stroma.",
        "magnification": "both", "class_association": "shared", "diagnostic_weight": 0.3},
    "Adipose tissue": {
        "description": "Mature adipocytes with peripherally displaced nuclei forming the fatty background of the breast parenchyma.",
        "magnification": "both", "class_association": "shared", "diagnostic_weight": 0.2},
}

DIAGNOSTIC_DISCRIMINATORS = {
    "Pleomorphic tumor cells": {
        "description": "Tumor cells display marked variation in size and shape, with irregular contours and disorganized cytoarchitecture, characteristic of high-grade ductal carcinoma.",
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 1.0},
    "Irregular nuclear contours": {
        "description": "Nuclei show jagged, wrinkled borders with coarse clumped chromatin and frequent atypical mitotic figures.",
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.95},
    "High nuclear-to-cytoplasmic ratio": {
        "description": "Tumor cells have large hyperchromatic nuclei and scant cytoplasm, reflecting aggressive cellular proliferation in invasive ductal carcinoma.",
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.9},
    "Prominent eosinophilic nucleoli": {
        "description": "Nuclei feature conspicuous central eosinophilic nucleoli, characteristic of high-grade invasive ductal carcinoma.",
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.9},
    "Increased mitotic activity": {
        "description": "Numerous mitotic figures, including abnormal tripolar and ring forms, are visible particularly at the invasive front of the tumor.",
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.9},
    "Ghost cell debris in necrotic foci": {
        "description": "Necrotic areas contain pyknotic nuclei, faded eosinophilic cytoplasm, and fragmented cell remnants forming amorphous debris.",
        "magnification": "20x", "class_association": "IDC", "diagnostic_weight": 0.7},
    "Small uniform tumor cells": {
        "description": "Tumor cells are monomorphic and small, with round-to-oval nuclei, inconspicuous nucleoli, and bland cytology typical of lobular carcinoma.",
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 1.0},
    "Round or oval bland nuclei": {
        "description": "Nuclear morphology is bland and regular, with smooth contours, fine evenly dispersed chromatin, and minimal atypia.",
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.95},
    "Minimal nuclear pleomorphism": {
        "description": "Cellular features are remarkably uniform with very limited variability in nuclear size, shape, or staining intensity.",
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Intracytoplasmic mucin vacuoles": {
        "description": "Clear cytoplasmic vacuoles, sometimes containing a central eosinophilic dot, displace nuclei to create a targetoid or signet-ring-like appearance characteristic of lobular carcinoma.",
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 1.0},
    "Lack of glandular lumen formation": {
        "description": "No glandular lumina or epithelial polarity are observed, a key distinguishing feature from invasive ductal carcinoma.",
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Nuclear monotony": {
        "description": "Cells exhibit strikingly uniform nuclear size, shape, and chromatin pattern, reflecting low-grade lobular morphology.",
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.9},
    "Clear cytoplasm with discrete vacuoles": {
        "description": "Cytoplasm appears pale to clear with discrete intracellular vacuoles, sometimes mistaken for benign histiocytic or stromal tissue at low magnification.",
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.85},
    "Discohesive single cells": {
        "description": "Tumor cells lack intercellular adhesion and junctions, appearing as isolated entities — the morphological correlate of E-cadherin loss in lobular carcinoma.",
        "magnification": "20x", "class_association": "ILC", "diagnostic_weight": 0.95},
}

ROUTING_TEMPLATES = [
    "a histopathology image showing {name}, characterized by {description}",
    "microscopic view of {name} pattern in breast tissue with {description}",
    "H&E stained section demonstrating {name} with features including {description}",
    "pathological finding of {name} in breast cancer specimen showing {description}",
]

CLASS_KNOWLEDGE_V2 = {
    "IDC": {"primary": "Invasive ductal carcinoma of the breast characterized by infiltrative tumor regions, malformed glandular or trabecular architecture, pleomorphic tumor cells with high nuclear-to-cytoplasmic ratio, prominent nucleoli, frequent mitoses, and a marked desmoplastic stromal reaction."},
    "ILC": {"primary": "Invasive lobular carcinoma of the breast characterized by single-file linear cords of small uniform discohesive tumor cells infiltrating diffusely through stroma, with bland round nuclei, intracytoplasmic mucin vacuoles, lack of gland formation, and preserved background architecture."},
}
...
def build_routing_concepts(target_scale=None):
    all_concepts = {}
    all_concepts.update(MORPHOLOGICAL_PATTERNS)
    all_concepts.update(DIAGNOSTIC_DISCRIMINATORS)
    all_concepts.update(MICROENVIRONMENT_COMPONENTS)
...
class UnbalancedOptimalTransport(nn.Module):
    ...
    def forward(self, F_feat, G_feat, a=None, b=None, return_pi=False):
        with torch.amp.autocast('cuda', enabled=False):
            ...
            if a is None: a = torch.ones(B, M, device=F_feat.device, dtype=torch.float32) / M
            if b is None: b = torch.ones(B, N, device=F_feat.device, dtype=torch.float32) / N
...
class SemanticWassersteinRouting(nn.Module):
    def __init__(self, feature_dim=512, hidden_dim=512, dropout=0.1, num_concepts=None):
        ...
        self.concept_importance = (
            nn.Parameter(torch.ones(num_concepts) * 0.5) if num_concepts is not None else None)
...
# ===========================================================================
# BRCA Concept-to-Class Mapper
# ===========================================================================
class BRCAConceptToClassMapper(nn.Module):
    def __init__(self, num_concepts, num_classes=2, class_associations=None,
...
# ===========================================================================
# Direct Patch-Class & Learnable Loss Weights
# ===========================================================================
class DirectPatchClassPathway(nn.Module):
    def __init__(self, feature_dim=512, num_classes=2, hidden_dim=128, dropout=0.5):
        super().__init__()
    ...
    def forward(self, patch_features, patch_counts=None):
        ...
        return torch.clamp(1 - F.softmax(self.patch_to_class(pooled), dim=-1), 0.0, 2.0)
...
class SinkhornBarycenter:
    ...
    def _sinkhorn_plan(C, epsilon=0.05, max_iter=50, convergence_thresh=1e-4):
        ...
        for it in range(max_iter):
            ...
            v = b / (K.T @ u + 1e-12)
            u = a / (K @ v + 1e-12)
            if it > 5 and it % 5 == 0 and (u - u_prev).abs().max() < convergence_thresh:
                break
    ...
    def compute_barycenter(cls, exemplars, weights=None, epsilon=0.05,
        ...
        init_costs = torch.tensor([
            cls.compute_cost_matrix(eucl_mean, exemplars[k]).sum().item() for k in range(K)])
...
# ===========================================================================
# BarycentreMemory — v3.1 RESUME-SAFE
# ===========================================================================
class BarycentreMemory(nn.Module):
    def __init__(self, k_shots=16, num_classes=2, num_patch_tokens=196,
        ...
        # v3.1 FIX: persistent state flags as registered buffers
    ...
    def restore_initialized_flag_from_buffers(self):
        if not self.memory_initialized and self.barycenters.abs().sum() > 1e-3:
            ...
            logger.info("[Memory] Backwards-compat restore: barycenters populated "
    ...
    def get_momentum(self):
        ...
        if   p < 0.3: s, e, lp = 0.50, 0.70, p / 0.3
        elif p < 0.7: s, e, lp = 0.70, 0.90, (p - 0.3) / 0.4
        else:         s, e, lp = 0.90, self.base_momentum, (p - 0.7) / 0.3
    ...
    def _balanced_assignments(self, dist_matrix, num_prototypes):
        ...
        for _ in range(20):
            ...
            for o in overfull:
                ...
                for idx in s_idx:
                    if sizes[o] <= target_sizes[o]: break
                    ex = o_idx[idx]
                    best_u, best_d = None, float('inf')
                    for u in underfull:
                        d = dist_matrix[ex, medoids[u]].item()
                        if d < best_d: best_d, best_u = d, u
                    if best_u is not None:
                        assignments[ex] = best_u
                        sizes[o] -= 1; sizes[best_u] += 1; moved = True
                        overfull  = [i for i, sz in enumerate(sizes) if sz > target_sizes[i]]
                        underfull = [i for i, sz in enumerate(sizes) if sz < target_sizes[i]]
                        if not overfull or not underfull: break
                if not overfull or not underfull: break
            if not moved: break
        return assignments
    ...
    def init_memory(self, visual_encoder, dataloader, device, scale_idx=0):
        print(f"Initializing Barycenter Memory ({self.k_shots} shots, "
              f"{self.num_prototypes} prototypes/class)...")
        visual_encoder.eval()
        ...
        for c in range(self.num_classes):
            if not class_patches[c]:
                ...
                print(f"  Class {c}: WARNING — no exemplars, random init"); continue
            ...
            if len(class_patches[c]) >= self.num_prototypes * 2:
                ...
                for ki in range(self.num_prototypes):
                    ...
                    if not members:
                        ...
                        self.prototype_weights[c, ki] = 1.0 / self.num_prototypes; continue
                    ce  = stacked[members]
                ...
                bary = F.normalize((1-self.euclidean_mix)*w_b + self.euclidean_mix*e_b, dim=-1)
                for ki in range(self.num_prototypes):
        ...
        print(f"Memory initialised: ({self.num_classes}, {self.num_prototypes}, "
    ...
    def ema_update(self, spatial_tokens, labels):
        ...
        for c in range(self.num_classes):
            ...
            for ki in range(self.num_prototypes):
                ...
                if bk.norm() < 1e-6:
                    proto_distances.append(torch.tensor(float('inf'))); continue
            ...
            temperature     = max(0.05, 0.5*(1.0 - self.current_epoch/max(self.ema_warmup_epochs*2, 1)))
            soft_weights    = F.softmax(-proto_distances / temperature, dim=0)
            for ki in range(self.num_prototypes):
                if self.barycenters[c, ki].norm() < 1e-6: continue
                ...
                if self.ema_noise_std > 0:
                    updated = F.normalize(updated + torch.randn_like(updated)*self.ema_noise_std*(1-w), dim=-1)
                self.update_magnitudes[c, ki] = 0.9*self.update_magnitudes[c, ki] + 0.1*(updated-old_bary).norm().item()
                self.total_updates_per_proto[c, ki] += 1
                self.barycenters[c, ki] = updated
            self._accumulation_buffer[c] = buf[-min(2, len(buf)):]
    ...
    def get_diagnostics(self):
        if not self.memory_initialized:
            return {}
        diag = {'momentum': self.get_momentum(),
                'total_updates': self._update_count,
                'batch_count':   self._batch_count}
        for c in range(self.num_classes):
            mags   = [self.update_magnitudes[c, ki].item() for ki in range(self.num_prototypes)]
            drifts = [(self.barycenters[c, ki] - self.initial_barycenters[c, ki]).norm().item()
                      for ki in range(self.num_prototypes)
...
class SWAModel:
    def __init__(self, model):
        self.model_state = {k: v.clone().float() for k, v in model.state_dict().items()}
        self.n_averaged  = 0
...
# PLIP Prompt Learner
# ===========================================================================
class PLIPSWRPromptLearner(nn.Module):
    def __init__(self, plip_model, class_names, num_soft_tokens=16, config=None):
        super().__init__()
        self.plip_model      = plip_model
        self.text_model      = plip_model.text_model
        self.tokenizer       = None
        ...
        # Hugging Face CLIP text tower internals.
        self.text_embeddings = self.text_model.embeddings
        self.token_embedding = self.text_embeddings.token_embedding
        self.position_embedding = self.text_embeddings.position_embedding
        self.encoder_layers = self.text_model.encoder.layers
        self.final_layer_norm = self.text_model.final_layer_norm
        self.text_projection = plip_model.text_projection

        for param in self.plip_model.parameters():
            param.requires_grad = False

        ctx_dim = int(getattr(text_cfg, "hidden_size", 512))
        ...
        self.routing_soft_ctx = nn.Parameter(
            torch.randn(self.num_routing_concepts, self.num_soft_tokens, ctx_dim,
                        dtype=torch.float32) * 0.01)

        self.slide_template_texts = build_class_prompts(class_names)
        self.slide_soft_ctx       = nn.Parameter(
            torch.randn(len(class_names), self.num_soft_tokens, ctx_dim,
                        dtype=torch.float32) * 0.01)

        self.register_buffer('routing_input_ids', torch.empty(0, dtype=torch.long), persistent=False)
        self.register_buffer('slide_input_ids',   torch.empty(0, dtype=torch.long), persistent=False)

        n_idc = sum(1 for a in self.class_associations if a == 'IDC')
        n_ilc = sum(1 for a in self.class_associations if a == 'ILC')
        n_x   = sum(1 for a in self.class_associations if a == 'shared')
        logger.info(
            f"PLIPSWRPromptLearner: {self.num_routing_concepts} concepts "
            f"(IDC={n_idc}, ILC={n_ilc}, Shared={n_x}), context_length={self.context_length}")
    ...
    def _build_4d_causal_mask(self, batch_size, seq_len, device, dtype):
    ...
    def _call_text_layer(self, layer, hidden_states, causal_attention_mask):
        try:
            out = layer(
                hidden_states,
                attention_mask=None,
            ...
            try:
                out = layer(hidden_states, None, causal_attention_mask, False)
            except TypeError:
                out = layer(hidden_states)
        return out[0] if isinstance(out, (tuple, list)) else out

    def _find_eos_pos(self, input_ids_1d):
        eos_id = None
        if self.tokenizer is not None:
            eos_id = getattr(self.tokenizer, "eos_token_id", None)
        if eos_id is None:
            eos_id = getattr(self.text_model.config, "eos_token_id", None)
        if eos_id is not None:
            pos = (input_ids_1d == int(eos_id)).nonzero(as_tuple=True)[0]
            if len(pos) > 0:
                return int(pos[0].item())
        pad_id = getattr(self.text_model.config, "pad_token_id", 1)
        non_pad = (input_ids_1d != int(pad_id)).nonzero(as_tuple=True)[0]
        if len(non_pad) > 0:
            return int(non_pad[-1].item())
        return int(input_ids_1d.argmax().item())
    ...
    def encode_text_with_soft_prompts(self, input_ids, soft_ctx):
        ...
        for i in range(soft_ctx.shape[0]):
            ...
            tok_emb = self.token_embedding(ids_i).to(device=device, dtype=soft_ctx.dtype)
            D       = tok_emb.shape[-1]

            eos_pos = self._find_eos_pos(ids_i[0])
            ...
            # Preserve EOS after inserting soft prompts.  Long pathology prompts are
            # truncated before EOS instead of dropping EOS accidentally.
            max_tmpl = max(0, ctx_len - 2 - self.num_soft_tokens)
            tmpl = tmpl[:, :max_tmpl, :]
            prompt = torch.cat([bos, soft_ctx[i:i+1], tmpl, eos], dim=1)
            new_eos = prompt.shape[1] - 1

            if prompt.shape[1] < ctx_len:
                pad = torch.zeros(1, ctx_len - prompt.shape[1], D, device=device, dtype=prompt.dtype)
                prompt = torch.cat([prompt, pad], dim=1)
            elif prompt.shape[1] > ctx_len:
                prompt = prompt[:, :ctx_len, :]
                new_eos = ctx_len - 1

            pos_ids = torch.arange(ctx_len, device=device, dtype=torch.long).unsqueeze(0)
            pos_emb = self.position_embedding(pos_ids).to(dtype=prompt.dtype)
            x = prompt + pos_emb
            causal_mask = self._build_4d_causal_mask(1, ctx_len, device, x.dtype)

            for layer in self.encoder_layers:
                x = self._call_text_layer(layer, x, causal_mask)
...
# ===========================================================================
# Deep Prompted PLIP Vision Transformer
# ===========================================================================
class DeepPromptedPLIPVisionTransformer(nn.Module):
    def __init__(self, plip_model, num_prompts=8, deep_depth=12,
                 use_gradient_checkpointing=False, num_scales=1,
                 use_scale_specific_prompts=False, disable_deep_prompts=False,
                 embedding_dim=512):
        super().__init__()
        self.plip_model    = plip_model
        self.vision_model  = plip_model.vision_model
        self.embeddings    = self.vision_model.embeddings
        self.encoder_layers = self.vision_model.encoder.layers
        self.pre_layrnorm  = self.vision_model.pre_layrnorm
        self.post_layernorm = self.vision_model.post_layernorm
        self.visual_projection = plip_model.visual_projection
        ...
        self.deep_depth                 = min(int(deep_depth), len(self.encoder_layers))
        ...
        self.proj_to_embed = nn.Linear(hd, embedding_dim)
    ...
    def _call_vision_layer(self, layer, hidden_states):
        try:
            out = layer(
                hidden_states,
                attention_mask=None,
    ...
    def _forward_block(self, block, x):
        if self.use_gradient_checkpointing and self.training:
            return grad_checkpoint(lambda inp: self._call_vision_layer(block, inp), x, use_reentrant=False)
        return self._call_vision_layer(block, x)

    def _stem_hf_clip(self, pixel_values):
        pixel_values = pixel_values.float()
        B = pixel_values.shape[0]
        emb = self.embeddings

        patch_embeds = emb.patch_embedding(pixel_values)          # B, hidden, grid, grid
        patch_embeds = patch_embeds.flatten(2).transpose(1, 2)    # B, tokens, hidden

        class_embeds = emb.class_embedding.to(device=pixel_values.device, dtype=patch_embeds.dtype)
        class_embeds = class_embeds.unsqueeze(0).unsqueeze(0).expand(B, 1, -1)
        x = torch.cat([class_embeds, patch_embeds], dim=1)

        if hasattr(emb, "position_ids"):
            pos_ids = emb.position_ids[:, :x.shape[1]].to(pixel_values.device)
        else:
            pos_ids = torch.arange(x.shape[1], device=pixel_values.device, dtype=torch.long).unsqueeze(0)
        pos_emb = emb.position_embedding(pos_ids).to(dtype=x.dtype)
        x = x + pos_emb
        x = self.pre_layrnorm(x)
        return x

    def forward(self, pixel_values, scale_idx=0):
        # Frozen HF PLIP vision tower + trainable deep prompt parameters.
        B = pixel_values.shape[0]
        prompts = self._get_prompts(scale_idx)

        x = self._stem_hf_clip(pixel_values)                      # B, L, D
        for i, block in enumerate(self.encoder_layers):
            use_p = (not self.disable_deep_prompts and i < self.deep_depth and prompts is not None)
            if use_p:
                prompt_tokens = prompts[i].to(device=x.device, dtype=x.dtype).unsqueeze(0).expand(B, -1, -1)
                x_with_prompt = torch.cat([x[:, :1, :], prompt_tokens, x[:, 1:, :]], dim=1)
                x_with_prompt = self._forward_block(block, x_with_prompt)
                x = torch.cat([x_with_prompt[:, :1, :], x_with_prompt[:, 1+self.num_prompts:, :]], dim=1)
            else:
                x = self._forward_block(block, x)

        cls = self.post_layernorm(x[:, 0, :])
        patch_tokens = self.post_layernorm(x[:, 1:, :]).float()
        cls_embed = self.visual_projection(cls)
        if cls_embed.shape[-1] != self.embedding_dim:
            cls_embed = self.proj_to_embed(cls)
        return cls_embed.float(), patch_tokens
...
# ===========================================================================
# Main Model — MultiScaleMUOT_PLIP
# ===========================================================================
class MultiScaleMUOT_PLIP(nn.Module):
    def __init__(self, config):
        ...
        print(f"Loading PLIP from Hugging Face Transformers: {config.plip_checkpoint} ...")
        self.plip_model, self.plip_preprocess_train, self.plip_preprocess_val = _load_plip_hf(config)
        self.plip_model = self.plip_model.float().eval().to(self.device)
        self.plip_img_size = int(config.plip_img_size)

        # Enforce PLIP/HF CLIP dimensions directly from config.json.
        hf_cfg = self.plip_model.config
        vcfg   = hf_cfg.vision_config
        tcfg   = hf_cfg.text_config
        config.embedding_dim  = int(getattr(hf_cfg, "projection_dim", config.embedding_dim))
        config.context_length = int(getattr(tcfg, "max_position_embeddings", config.context_length))

        patch_size = getattr(vcfg, "patch_size", 32)
        if isinstance(patch_size, (tuple, list)):
            patch_size = patch_size[0]
        image_size = int(getattr(vcfg, "image_size", self.plip_img_size))
        self.plip_img_size = image_size
        config.plip_img_size = image_size
        if config.num_patch_tokens is None:
            config.num_patch_tokens = (image_size // int(patch_size)) ** 2
        if config.vit_hidden_dim is None:
            config.vit_hidden_dim = int(getattr(vcfg, "hidden_size", 768))
        print(f"PLIP ViT: patch_size={patch_size}, {config.num_patch_tokens} spatial tokens, "
              f"hidden_dim={config.vit_hidden_dim}, embed_dim={config.embedding_dim}, "
              f"context_length={config.context_length}")

        self.visual_encoder = DeepPromptedPLIPVisionTransformer(
            self.plip_model,
            num_prompts=config.num_visual_prompts,
        ...
        class_associations   = [all_concepts[n].get("class_association", "shared") for n in all_concepts]
        ...
        for s_idx, sn in enumerate(self.scale_names):
            ...
            if use_swr:
                agg = SemanticWassersteinAggregation(config.embedding_dim, config.agg_dropout, num_routing_concepts)
            elif config.aggregation_method == 'prompt':
                agg = PromptBasedAggregation(config.embedding_dim, config.agg_num_prompts,
                                             config.agg_num_heads, config.agg_dropout, config.agg_num_layers)
            else:
                agg = WSIAttentionAggregation(config.embedding_dim, config.agg_num_heads, config.agg_dropout)
            self.scale_aggregators.append(agg)

        self.scale_fusion = MultiScaleFusion(
            config.embedding_dim, self.num_scales, config.multi_scale_fusion, config.agg_dropout)
        ...
        self.nested_prompt_learner = PLIPSWRPromptLearner(
            self.plip_model, config.class_names, config.num_soft_tokens, config)
        self.tokenizer = _get_plip_tokenizer(config)
        self.nested_prompt_learner.set_tokenizer(self.tokenizer)

        if self._use_pl:
            self.tissue_mapper = BRCAConceptToClassMapper(
                num_routing_concepts, config.num_classes, class_associations,
                config.agg_hidden_dim, config.agg_dropout, class_to_idx=config.class_to_idx)

        if config.use_direct_patch_class:
            self.direct_patch_class = DirectPatchClassPathway(
                config.embedding_dim, config.num_classes, config.agg_hidden_dim, config.agg_dropout)
        ...
        for p in self.plip_model.parameters():
    ...
    def init_memory(self, dataloader):
        if self.memory is not None:
            self.memory.init_memory(self.visual_encoder, dataloader, self.device,
    ...
    def _encode_scale(self, patches, patch_counts, scale_idx, return_spatial=False,
        ...
        # Pre-extracted feature tensors (B, N, D)
        if (isinstance(patches, torch.Tensor) and patches.dim() == 3
        ...
        if return_spatial and raw_spatial is not None:
            ...
            tile_mask            = torch.arange(N, device=patches.device).unsqueeze(0) < patch_counts.unsqueeze(1)
            denom                = patch_counts.float().unsqueeze(-1).unsqueeze(-1).clamp(min=1)
            slide_spatial        = F.normalize(
                (local_norm * tile_mask.unsqueeze(-1).unsqueeze(-1).float()).sum(dim=1) / denom, dim=-1)
            del raw_spatial
    ...
    def forward_wsi(self, multi_patches, labels=None, multi_patch_counts=None):
        ...
        memory_spatial = memory_patch_feat = memory_patch_counts = memory_pertile_spatial = None
        ...
        for s_idx, sn in enumerate(self.scale_names):
            ...
            (slide_feat, patch_feat, slide_spatial,
             pertile_spatial, updated_counts) = self._encode_scale(
                ...
                return_spatial=is_mem, text_features=scale_text,
                ...
                for k in range(self.config.num_classes):
                    ...
                    for p in range(self.memory.num_prototypes):
                        ...
                        if bary_kp.norm() < 1e-6:
                            proto_dists.append(torch.ones(B, device=device)); continue
        ...
        if labels is not None:
            ...
            def margin_loss(d, labs):
                if d is None: return torch.tensor(0.0, device=device)
                dp = d[bi, labs]
            ...
            lm_d = margin_loss(d_direct,      labels) if d_direct is not None else torch.tensor(0.0, device=device)

            pdl = torch.tensor(0.0, device=device)
            if self.memory is not None and self.config.prototype_diversity_weight > 0:
                pdl = self.memory.compute_prototype_diversity_loss() * self.config.prototype_diversity_weight
            ...
            if self.loss_weighter is not None:
                ...
                total_loss = loss_ce + 0.1*lm_s + 0.1*lm_I + 0.05*lm_p + 0.05*lm_d + cp + pdl
...
# ===========================================================================
# DATA — Label CSV, Indexing, Splits, Datasets
# ===========================================================================
class TCGAIDParser:
...
def _read_label_csv(label_csv_path):
    if not os.path.exists(label_csv_path):
        raise FileNotFoundError(f"Label CSV not found: {label_csv_path}")
    df = None
    for sep in [None, '\t', ',']:
        try:
            kwargs = {'engine': 'python'} if sep is None else {'sep': sep}
            df = pd.read_csv(label_csv_path, **kwargs)
            if 'submitter_id' in df.columns and 'label' in df.columns:
                break
        except Exception:
            df = None
    if df is None or 'submitter_id' not in df.columns or 'label' not in df.columns:
        raise RuntimeError(
            f"Could not parse {label_csv_path} — need columns `submitter_id` and `label`.")
    return {str(sid).strip(): str(lbl).strip()
            for sid, lbl in zip(df['submitter_id'], df['label'])}


def index_brca_dataset(brca_root, plip_feats_root, label_csv_path, class_to_idx,
                       scales=('5x', '20x'), min_patches_per_scale=None):
    if min_patches_per_scale is None:
        min_patches_per_scale = {s: 10 for s in scales}

    pid_to_label = _read_label_csv(label_csv_path)
    logger.info(f"Loaded {len(pid_to_label)} patient labels from {label_csv_path}")

    brca_root_p = Path(brca_root)
    slides_root = None
    for candidate in [brca_root_p / "pyramid", brca_root_p]:
        if candidate.exists() and any(d.is_dir() for d in candidate.iterdir()):
            slides_root = candidate
            break
    if slides_root is None:
        logger.error(f"No slide directories found under {brca_root}")
        return []

    feats_root_p = Path(plip_feats_root)
    indexed      = []
    stats        = defaultdict(int)
    slide_dirs   = sorted([d for d in slides_root.iterdir() if d.is_dir()])
    logger.info(f"BRCA: scanning {len(slide_dirs)} slide directories under {slides_root}")

    for slide_dir in tqdm(slide_dirs, desc="Indexing BRCA"):
        sid = slide_dir.name
        pid = TCGAIDParser.parse(sid)['patient_id']

        if pid not in pid_to_label:
            stats['no_label_in_csv'] += 1
            continue

        label_name = pid_to_label[pid]
        if label_name not in class_to_idx:
            stats[f'unknown_label_{label_name}'] += 1
            continue
        ...
        for s in scales:
            if s == '5x':
                s_dir = slide_dir / "5x"
                if not s_dir.exists():
                    ok = False; stats[f'{label_name}_no_5x_dir'] += 1; break
                paths = _list_images_in(s_dir)
                if len(paths) < min_patches_per_scale.get(s, 10):
                    ok = False; stats[f'{label_name}_too_few_5x'] += 1; break
                scale_paths[s] = paths
                stats[f'{label_name}_img_5x'] += 1

            elif s == '20x':
                # Supported PLIP 20x layouts:
                #   <plip_feats_root>/<slide_id>/20x/*.pt
                #   <plip_feats_root>/<slide_id>/*.pt
                #   <plip_feats_root>/<slide_id>.pt
                #   <plip_feats_root>/<slide_id>/features_20x.pt
                feat_dir = None
                for fdc in [feats_root_p / sid / "20x",
                            feats_root_p / sid,
                            feats_root_p / "pyramid" / sid / "20x",
                            feats_root_p / "pyramid" / sid]:
                    if fdc.exists() and fdc.is_dir():
                        pts = list_pt_files_in_directory(fdc)
                        if pts:
                            feat_dir = fdc
                            break
                if feat_dir is not None:
                    pt_files = list_pt_files_in_directory(feat_dir)
                    # If this directory has a single aggregated tensor, _load() handles (N,512).
                    if len(pt_files) >= min_patches_per_scale.get(s, 30) or len(pt_files) == 1:
                        scale_paths[s] = pt_files[0] if len(pt_files) == 1 else pt_files
                        stats[f'{label_name}_feat_20x'] += 1
                    else:
                        ok = False; stats[f'{label_name}_too_few_20x_feats'] += 1; break
                else:
                    single_candidates = [
                        feats_root_p / f"{sid}.pt",
                        feats_root_p / sid / "features_20x.pt",
                        feats_root_p / sid / "features_20x_plip.pt",
                        feats_root_p / "pyramid" / sid / "features_20x.pt",
                    ]
                    single_pt = next((c for c in single_candidates if c.exists()), None)
                    if single_pt is not None:
                        scale_paths[s] = str(single_pt)
                        stats[f'{label_name}_feat_20x_single'] += 1
                    else:
                        ok = False; stats[f'{label_name}_missing_20x_feat'] += 1; break

        if not ok or not scale_paths:
            stats[f'{label_name}_skipped'] += 1
            continue
        ...
        indexed.append({
            ...
            'num_patches':      len(scale_paths.get('5x', [])),
            'available_scales': list(scale_paths.keys()),
            'patient_id':       pid,
        })

    logger.info(f"BRCA indexing summary: {dict(stats)}")
    return indexed


def make_patient_level_splits(indexed_data, train_ratio=0.7, val_ratio=0.15,
                              test_ratio=0.15, seed=82, num_classes=2):
    ...
    for cls in range(num_classes):
        ...
        for pid in pids[:n_train]:            train_slides.extend(patient_to_slides[pid])
        for pid in pids[n_train:n_train+n_val]: val_slides.extend(patient_to_slides[pid])
        for pid in pids[n_train+n_val:]:      test_slides.extend(patient_to_slides[pid])
...
def save_splits_to_json(train_data, val_data, test_data, json_path):
    def slim(items):
        return [{'slide_id': it['slide_id'], 'label': it['label'],
                 'class_name': it['class_name'], 'patient_id': it['patient_id']} for it in items]
    with open(json_path, 'w') as f:
        json.dump({'train': slim(train_data), 'val': slim(val_data), 'test': slim(test_data)}, f, indent=2)
...
class MultiScaleWSIDataset(Dataset):
    def __init__(self, indexed_data, transform=None, is_train=True, config=None):
        ...
        self.img_size   = int(getattr(config, 'plip_img_size', 224))
        ...
        for it in self.data:
            ...
            for s in self.scales:
                ...
                if s == '20x' and isinstance(val, list) and all(p.endswith('.pt') for p in val):
                    ...
                    sel = random.sample(val, mx) if (len(val) > mx and self.is_train) \
                        else ([val[i] for i in np.linspace(0, len(val)-1, mx, dtype=int)] if len(val) > mx else list(val))
                    if self.is_train: random.shuffle(sel)
                    ...
                    sel   = random.sample(paths, mx) if (len(paths) > mx and self.is_train) \
                        else ([paths[i] for i in np.linspace(0, len(paths)-1, mx, dtype=int)] if len(paths) > mx else list(paths))
                    if self.is_train: random.shuffle(sel)
                    sample['scale_paths'][s] = sel
    ...
    def _coerce_plip_feature(self, feat, src=""):
        """Coerce PLIP feature tensors to float32 shape (N, 512) or (512,)."""
        if isinstance(feat, dict):
            for k in ("features", "feats", "embeddings", "image_features", "x"):
                if k in feat:
                    feat = feat[k]
                    break
        ...
        if not torch.is_tensor(feat):
            raise RuntimeError(f"Unsupported feature object from {src}: {type(feat)}")
        feat = feat.detach().cpu().float()
        if feat.dim() == 3 and feat.shape[0] == 1:
            feat = feat.squeeze(0)
        if feat.dim() == 2 and feat.shape[-1] == self.config.embedding_dim:
            return feat.contiguous()
        if feat.dim() == 1 and feat.shape[0] == self.config.embedding_dim:
            return feat.contiguous()
        raise RuntimeError(f"Unexpected PLIP feature tensor shape {tuple(feat.shape)} from {src}; expected (*, {self.config.embedding_dim})")

    def _load(self, scale_item):
        if isinstance(scale_item, str) and scale_item.endswith('.pt'):
            feat = self._coerce_plip_feature(torch.load(scale_item, map_location='cpu'), scale_item)
            if feat.dim() == 1:
            ...
            features = []
            for p in scale_item:
                feat = self._coerce_plip_feature(torch.load(p, map_location='cpu'), p)
                if feat.dim() == 1:
                    features.append(feat)
                elif feat.dim() == 2 and feat.shape[0] == 1:
                    features.append(feat.squeeze(0))
                else:
                    # A per-patch file unexpectedly contains multiple features; keep them all.
                    features.extend([row for row in feat])
            return torch.stack(features, dim=0).float() if features else torch.zeros(0, self.config.embedding_dim, dtype=torch.float32)
        else:
            patches = []
            ...
            for p in scale_item:
                try:
                    with Image.open(p) as img:
                        ...
                        patches.append(self.transform(img) if self.transform else transforms.ToTensor()(img))
                except Exception:
                    continue
            return torch.stack(patches, dim=0) if patches else torch.zeros(1, 3, self.img_size, self.img_size)

    def __getitem__(self, idx):
        s = self.epoch_samples[idx]
        return ({sc: self._load(s['scale_paths'][sc]) for sc in self.scales}, s['label'], s['slide_id'])
...
def collate_multiscale_batch(batch):
    ...
    for s in scales:
        ...
        if isinstance(first, torch.Tensor) and first.dim() == 2:
            ...
            for i, t in enumerate(ts): pad[i, :t.shape[0]] = t
            mp[s] = pad; mc[s] = cs
        else:
            ts  = [b[0][s] for b in batch]
            cs  = torch.tensor([t.shape[0] for t in ts], dtype=torch.long)
...
class FilteredMemoryDataset(Dataset):
    def __init__(self, filtered_data, memory_scale, transform=None, max_per_slide=3, img_size=224):
        self.transform = transform
        ...
        for item in filtered_data:
            ...
            if isinstance(val, list) and all(isinstance(p, str) and not p.endswith('.pt') for p in val):
    ...
    def __getitem__(self, idx):
        ...
        try:
            with Image.open(path) as img:
                img = img.convert('RGB')
                if img.size != tgt: img = img.resize(tgt, Image.Resampling.BILINEAR)
...
def create_balanced_sampler(indexed_data, strategy='class_and_sqrt_size'):
    ...
    cf     = defaultdict(int)
    for l in labels: cf[l] += 1
    cw      = {c: 1.0/n for c, n in cf.items()}
    weights = []
    for l, c in zip(labels, counts):
        w = cw[l]
        if   strategy == 'class_and_sqrt_size':  w *= 1.0 / max(np.sqrt(c), 1)
        elif strategy == 'class_and_inverse':     w *= 1.0 / max(c, 1)
        weights.append(w)
    t       = sum(weights)
...
def create_few_shot_subset(indexed_data, k_per_class):
    ci = defaultdict(list)
    for i, it in enumerate(indexed_data): ci[it['label']].append(i)
    sel = []
...
def evaluate_wsi(model, val_loader, config):
    model.eval()
    all_p, all_l, all_preds = [], [], []
    total_loss, num_batches, cor, tot = 0.0, 0, 0, 0
    with torch.no_grad():
        for bd in tqdm(val_loader, desc="Validation"):
            ...
            cor += (pred == lb).sum().item(); tot += lb.size(0)
    ...
    bal_acc       = balanced_accuracy_score(all_l, all_preds) if all_l else 0.0
    all_probs_arr = np.concatenate(all_p, axis=0) if all_p else np.zeros((0, config.num_classes))
    try:
        auc = roc_auc_score(all_l, all_probs_arr[:, 1]) if config.num_classes == 2 \
              else roc_auc_score(all_l, all_probs_arr, multi_class='ovr', average='macro')
    except Exception:
        auc = 0.0
    try:
    ...
    return {'acc': acc, 'auc': auc, 'f1': f1,
            ...
            'all_labels': all_l, 'all_preds': all_preds}
...
def evaluate_test_full_report(model, test_loader, config, output_dir="./test_results"):
    ...
    try:
        auc = roc_auc_score(all_labels, all_probs[:, 1]) if config.num_classes == 2 \
              else roc_auc_score(all_labels, all_probs, multi_class='ovr', average='macro')
    except Exception:
        auc = 0.0
    try:
    ...
    print(f"\n{'='*60}\nTEST REPORT\n{'='*60}\n{report}")
    print(f"Accuracy: {acc:.4f} | Bal Acc: {bal_acc:.4f} | AUC: {auc:.4f} | "
          f"AP: {ap:.4f} | F1(macro): {f1_m:.4f} | F1(weighted): {f1_w:.4f}")
    print(f"Confusion Matrix:\n{cm}")
    df_dict = {"slide_id": all_slide_ids, "true_label": all_labels,
    ...
    pd.DataFrame(df_dict).to_csv(os.path.join(output_dir, "test_predictions.csv"), index=False)
    summary = {"accuracy": float(acc), "balanced_accuracy": float(bal_acc),
               "auc": float(auc), "average_precision_macro": float(ap),
...
def load_checkpoint(path, model, optimizer=None, scheduler=None, scaler=None,
    ...
    if scheduler  is not None and 'scheduler_state_dict'  in ckpt: scheduler.load_state_dict(ckpt['scheduler_state_dict'])
    if scaler     is not None and 'scaler_state_dict'     in ckpt: scaler.load_state_dict(ckpt['scaler_state_dict'])
...
def train_one_epoch(model, train_loader, optimizer, scaler, config, epoch):
    ...
    for pass_idx in range(num_passes):
        ...
        for bi, bd in enumerate(pbar):
            ...
            pred  = torch.argmax(out['probs'], dim=1)
            tcor += (pred == lb).sum().item(); ttot += lb.size(0)
            el   += out['total_loss'].item(); ec += out['loss_ce'].item(); nb += 1
            for k in ['loss_margin_slide', 'loss_margin_I', 'loss_margin_patch',
                      'loss_margin_direct', 'loss_proto_diversity']:
                if k in out: loss_details[k] += out[k].item()
            pbar.set_postfix(loss=f"{out['total_loss'].item():.4f}",
                             acc=f"{tcor/max(ttot,1):.3f}")

    el /= max(nb, 1); ec /= max(nb, 1); ta = tcor / max(ttot, 1)
    for k in loss_details: loss_details[k] /= max(nb, 1)
...
# ===========================================================================
def parse_args():
    p = argparse.ArgumentParser(description="MultiScale MUOT-PLIP BRCA 2-class (IDC/ILC)")
    p.add_argument('--resume',           action='store_true', help='Resume from last_checkpoint.pth')
    p.add_argument('--resume_from',      type=str, default=None, help='Explicit checkpoint path')
    p.add_argument('--rebuild_splits',   action='store_true', help='Rebuild train/val/test splits')
    p.add_argument('--k',                type=int, default=None, help='Override few_shot_k_train')
    p.add_argument('--epochs',           type=int, default=None, help='Override num_epochs')
    p.add_argument('--brca_root',        type=str, default=None, help='BRCA WSI root directory')
    p.add_argument('--plip_feats_root',  type=str, default=None, help='PLIP 20x features root')
    p.add_argument('--label_csv',        type=str, default=None, help='Label CSV (submitter_id, label)')
    return p.parse_args()
...
def main(cfg=None, args=None):
    ...
    if args.k                is not None: cfg.few_shot_k_train  = args.k
    if args.epochs           is not None: cfg.num_epochs         = args.epochs
    if args.brca_root        is not None: cfg.brca_root          = args.brca_root
    if args.plip_feats_root  is not None: cfg.plip_feats_root    = args.plip_feats_root
    if args.label_csv        is not None: cfg.label_csv          = args.label_csv
    ...
    print("\n" + "=" * 80)
    print("MULTI-SCALE MUOT-PLIP — TCGA-BRCA 2-class (IDC/ILC) v3.1")
    print(f"  k={cfg.few_shot_k_train} | GPU: cuda:{cfg.gpu_device_id}")
    print(f"  Routing concepts  : {len(build_routing_concepts())} (BRCA-specific)")
    print(f"  Memory scale      : {cfg.memory_scale}")
    print(f"  WSI root          : {cfg.brca_root}")
    print(f"  Feats root        : {cfg.plip_feats_root}")
    print(f"  Label CSV         : {cfg.label_csv}")
    print(f"  Save dir          : {cfg.save_dir}")
    print("=" * 80 + "\n")

    # ---------- Indexing ----------
    print(f"\n[Indexing] Walking {cfg.brca_root} ...")
    all_indexed = index_brca_dataset(
        cfg.brca_root, cfg.plip_feats_root, cfg.label_csv, cfg.class_to_idx,
        scales=tuple(cfg.scales), min_patches_per_scale=cfg.scale_min_patches)
    if not all_indexed:
        logger.error("No slides indexed — check BRCA paths and label CSV.")
        return None

    n_per_class = defaultdict(int)
    for it in all_indexed: n_per_class[it['class_name']] += 1
    print(f"  Slides per class: {dict(n_per_class)}  |  Total: {len(all_indexed)}")
    ...
    for name, split in [('train', train_full), ('val', val_full), ('test', test_full)]:
        pc = defaultdict(int)
        for it in split: pc[it['class_name']] += 1
        print(f"  {name:5s}: {len(split):4d} slides  per-class={dict(pc)}")
    ...
    for item in train_data: fs_counts[cfg.idx_to_class[item['label']]] += 1
    logger.info(f"Final training set (k={cfg.few_shot_k_train}): {dict(fs_counts)}")

    # ---------- Transforms ----------
    MEAN = (0.48145466, 0.4578275, 0.40821073)  # PLIP/CLIP mean
    STD  = (0.26862954, 0.26130258, 0.27577711)  # PLIP/CLIP std
    ...
    # ---------- Model ----------
    model = MultiScaleMUOT_PLIP(cfg).to(cfg.device)
    ...
    for n, p in model.named_parameters():
        ...
        if not matched: grp['other'].append(p)
    ...
    if do_resume:
        ...
        if model.memory is not None and not model.memory.memory_initialized:
            print("\n>> Memory not initialized after load → rebuilding ...")
            mem_ds = FilteredMemoryDataset(train_ds.data, cfg.memory_scale, vt,
                                           img_size=cfg.plip_img_size)
            mem_dl = DataLoader(mem_ds, batch_size=32, shuffle=False,
                                num_workers=2, pin_memory=True)
            model.init_memory(mem_dl)
        else:
            updates = model.memory._update_count if model.memory else 'N/A'
            print(f">> Memory already initialized (updates so far: {updates})")
        print()
    else:
        mem_ds = FilteredMemoryDataset(train_ds.data, cfg.memory_scale, vt,
                                       img_size=cfg.plip_img_size)
        ...
        for epoch in range(start_epoch, cfg.num_epochs):
            ...
            if 'ema_momentum' in train_metrics:
                ...
                for c in range(cfg.num_classes):
                    ...
                    status    = ("⚠️ FROZEN"   if avg_mag   < 0.001 else
                                 "⚠️ COLLAPSED" if inter_sim > 0.8   else
                                 "⚠️ DRIFTED"  if avg_drift > 0.5   else "✓")
                    print(f"    {cfg.idx_to_class[c]}: mag={avg_mag:.5f}  "
                          f"drift={avg_drift:.4f}  sim={inter_sim:.4f}  "
                          f"buf={buf_size}  {status}")

            if cfg.swa_enabled and swa_model is not None and epoch >= cfg.swa_start_epoch:
            ...
            if (epoch + 1) % cfg.val_frequency == 0 or epoch == cfg.num_epochs - 1:
                ...
                gap_warn = (f" ⚠️ GAP={overfit_gap:.3f}" if overfit_gap > cfg.overfit_gap_threshold else "")
