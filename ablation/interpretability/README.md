...
- **The full model has SWR active.** For ablations like `no_swr` or
  `no_patch_prompts` the routing pathway either doesn't exist or
  receives no concept text. The transport-map script will still run
  on those checkpoints but the heatmaps may be uninformative — for
  the paper's interpretability story you almost certainly want the
  `full_model` checkpoint.

- **The script picks slides at random** within each class
  (with a fixed `--seed`). Re-running with a different seed gives
  different examples — pick the most visually striking ones for the
  paper.

- **Stitching ≠ original WSI.** The mosaic is a thumbnail collage of
  the patches on disk. The arrangement reflects the patches' grid
  coordinates but not their absolute pixel positions on the original
  slide. For most pathology figures this is plenty.

- The transport-map script visualizes the **5×** scale by default
  because that's where (a) raw images are available and (b) the
  spatial structure is easiest to read. To visualize 20× you would
  need raw 20× images on disk; the current pipeline only has 20×
  features (`.pt`), so 20× will be skipped with a clear warning.
